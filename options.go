...
type SendOptions struct {
	...
	// TopicID is the ID of the group topic (thread) to which the message will be sent. Supports only supergroups and channels.
	TopicID int
}
...
func (b *Bot) embedSendOptions(params map[string]string, opt *SendOptions) {
	...
	if opt.TopicID != 0 {
		params["message_thread_id"] = strconv.Itoa(opt.TopicID)
	}
}
